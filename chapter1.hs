# Haskell-Chapter-1
module Main where

---------------------------
-- HC1T1 - Task 1
-- Function Composition
---------------------------

double :: Int -> Int
double x = x * 2

increment :: Int -> Int
increment x = x + 1

-- Apply double first, then increment
doubleThenIncrement :: Int -> Int
doubleThenIncrement = increment . double


---------------------------
-- HC1T2 - Task 2
-- Pure Function Example
---------------------------

circleArea :: Floating a => a -> a
circleArea r = pi * r * r   -- Pure function, no external state


---------------------------
-- HC1T3 - Task 3
-- Checking if Number > 18
---------------------------

greaterThan18 :: Int -> Bool
greaterThan18 x = x > 18


---------------------------
-- HC1T4 - Task 4
-- Player Data Processing
---------------------------

-- Extract only names from list of (name, score)
extractPlayers :: [(String, Int)] -> [String]
extractPlayers players = [name | (name, _) <- players]

-- Sort players by score descending
sortByScore :: [(String, Int)] -> [(String, Int)]
sortByScore players = reverse (sortOn snd players)

-- Return top 3 players
topThree :: [(String, Int)] -> [(String, Int)]
topThree players = take 3 players

-- Compose functions into getTopThreePlayers
getTopThreePlayers :: [(String, Int)] -> [(String, Int)]
getTopThreePlayers = topThree . sortByScore


---------------------------
-- HC1T5 - Task 5
-- Laziness & Infinite List
---------------------------

infiniteNumbers :: [Int]
infiniteNumbers = [1..]     -- Infinite list

firstN :: Int -> [Int]
firstN n = take n infiniteNumbers


---------------------------
-- HC1T6 - Task 6
-- Type Signatures
---------------------------

addNumbers :: Int -> Int -> Int
addNumbers a b = a + b


---------------------------
-- HC1T7 - Task 7
-- Fahrenheit to Celsius
---------------------------

fToC :: Floating a => a -> a
fToC f = (f - 32) * (5 / 9)


---------------------------
-- HC1T8 - Task 8
-- Higher-Order Functions
---------------------------

applyTwice :: (a -> a) -> a -> a
applyTwice f x = f (f x)


---------------------------
-- MAIN (Testing outputs)
---------------------------

main :: IO ()
main = do
    putStrLn "HC1T1 doubleThenIncrement 5:"
    print (doubleThenIncrement 5)

    putStrLn "\nHC1T2 circleArea 10:"
    print (circleArea 10)

    putStrLn "\nHC1T3 greaterThan18 20:"
    print (greaterThan18 20)

    putStrLn "\nHC1T4 getTopThreePlayers:"
    print (getTopThreePlayers [("Adiv", 30), ("Neo", 50), ("Liam", 45), ("Timo", 10)])

    putStrLn "\nHC1T5 first 10 infinite numbers:"
    print (firstN 10)

    putStrLn "\nHC1T6 addNumbers 5 7:"
    print (addNumbers 5 7)

    putStrLn "\nHC1T7 fToC 100F:"
    print (fToC 100)

    putStrLn "\nHC1T8 applyTwice (*2) 5:"
    print (applyTwice (*2) 5)
